...
if (isset($_GET['id'])) {
  ...
  $hewan = select("SELECT * FROM pets WHERE id=$id")[0];

  // Ambil nama jurusan berdasarkan id_jurusan
  $id_pet = $hewan['id_owner'];
  $pets = select("SELECT nama FROM owners WHERE id=$id_pet")[0]['nama'];

  if (isset($_POST['ubah'])) {
    if (update_peliharaan($_POST) > 0) {
      echo "<script>
  ...
  <div id="wrapper">
    ...
    <div id="content-wrapper" class="d-flex flex-column">
      ...
      <div id="content">
        ...
        <h1 class="h3 mb-0 text-gray-800">Edit Data Peliharaan</h1>
    </div>

    <form action="" method="POST">
        <input type="hidden" class="form-control" id="id" name="id" value="<?= $hewan['id']; ?>" >
        <div class="mb-3">
        <input type="hidden" readonly class="form-control" id="id_owner" name="id_owner" value="<?= $hewan['id_owner']; ?>">
        </div>
      <div class="mb-3">
        <label for="nama_owner" class="form-label">Pemilik</label>
        <input type="text" readonly class="form-control" id="id_owner" value="<?= $pets; ?>">
      </div>
          
        <div class="mb-3">
          <label for="Hewan" class="form-label">Nama Hewan</label>
          <input type="text" class="form-control" id="nama_hewan" name="nama_hewan" value="<?= $hewan['nama_hewan']; ?>">
          </div>
          ...
          <input type="text" class="form-control" id="ras" name="ras" value="<?= $hewan['ras']; ?>">
            </div>
        
          <div class="form-group mb-3">
          <label for="jk">Jenis Kelamin Hewan</label>
          <input class="form-control" id="jenis_kelamin" name="jenis_kelamin" value="<?= $hewan['jenis_kelamin']; ?>">
          <small class="form-text text-muted">Masukkan 'Jantan' untuk Laki-laki atau 'Betina' untuk Perempuan.</small>
        </div>

        <div class="mb-3">
          <label for="Ras" class="form-label">Tanggal Lahir Hewan</label>
          <input type="date" class="form-control" id="tanggal_lahir" name="tanggal_lahir" value="<?= $hewan['tanggal_lahir']; ?>">
        </div>
